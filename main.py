...
class MyPlayer(Player):
    ...
    def preflop_move(
        ...
        if to_call == 0:
            if strength >= 0.84:
                return self.aggressive_action(valid_moves, min_bet, max_bet, 400, Move.CHECK)
        ...
        if strength >= 0.9 or (strength >= 0.84 and stack_pressure > 0.18):
            if Move.ALL_IN in valid_moves and max_bet <= 2600:
                return Move.ALL_IN
            target = max(min_bet, int((to_call + self.pot_commitment) * 2.6))
        ...
        if strength >= 0.66 and bet_size_bb <= 6.5 and Move.RAISE in valid_moves:
            target = max(min_bet, int((to_call + self.pot_commitment) * 2.5))
            return self.aggressive_action(valid_moves, min_bet, max_bet, target, Move.CALL)
        ...
        if strength >= defend_threshold and to_call <= max(600, int(max_bet * 0.14)) and Move.CALL in valid_moves:
            return Move.CALL
    ...
    def move(
        ...
        if to_call > 0:
            if strength >= 0.78:
                target = max(min_bet, int(opp_commitment + max(to_call * 1.8, estimated_pot * 0.65)))
                return self.aggressive_action(valid_moves, min_bet, max_bet, target, Move.CALL)

            if strength >= 0.68 and not facing_heat and len(community_cards) < 5 and Move.RAISE in valid_moves:
                target = max(min_bet, int(opp_commitment + max(to_call * 1.35, estimated_pot * 0.4)))
                return self.aggressive_action(valid_moves, min_bet, max_bet, target, Move.CALL)
            ...
            if strength >= call_threshold and to_call <= max(700, int(max_bet * 0.18)) and Move.CALL in valid_moves:
                return Move.CALL
        ...
        if len(community_cards) == 3 and strength >= 0.52 and not opp_loose:
            target = max(min_bet, int(estimated_pot * 0.42))
            return self.aggressive_action(valid_moves, min_bet, max_bet, target, Move.CHECK)

        if strong_draw and len(community_cards) < 5 and not opp_loose:
            target = max(min_bet, int(estimated_pot * 0.48))
            return self.aggressive_action(valid_moves, min_bet, max_bet, target, Move.CHECK)
