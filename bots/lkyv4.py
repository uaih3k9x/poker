...
class MyPlayer(Player):
    ...
    def _sync_street_state(self, community_cards, round_history):
        ...
        if board_len != self._last_board_len:
            # New street: carry over previous street's commitments
            if self._last_board_len > 0 or board_len == 0:
                # Calculate previous street total (if any)
    ...
    def preflop_move(self, valid_moves, min_bet, max_bet, to_call, pot_odds, round_history):
        ...
        regime = self.stack_regime(max_bet)

        # === Short stack regime: jam/fold ===
        if regime == 'short':
            if facing_allin:
                return Move.CALL if strength >= 0.78 and Move.CALL in valid_moves else Move.FOLD
            if to_call == 0:
                if self._is_button:
                    if strength >= 0.50:
                        return Move.ALL_IN if Move.ALL_IN in valid_moves else Move.CHECK
                else:
                    if strength >= 0.60:
                        return Move.ALL_IN if Move.ALL_IN in valid_moves else Move.CHECK
            ...
            # Facing raise
            if strength >= 0.70:
                return Move.ALL_IN if Move.ALL_IN in valid_moves else Move.CALL
            if strength >= 0.55 and to_call <= max_bet * 0.3:
                return Move.CALL if Move.CALL in valid_moves else Move.FOLD
            return Move.FOLD

        # === Normal / deep stack regime ===
        if facing_allin:
    ...
    def postflop_move(self, community_cards, valid_moves, round_history,
        ...
        # === Facing a bet / raise ===
        if to_call > 0:
            # Hard fold rules with pair_flags
            if facing_heat and top_pair and not overpair and len(community_cards) >= 4 and equity < 0.72:
                return Move.CHECK if Move.CHECK in valid_moves else Move.FOLD
            if is_river and top_pair and facing_allin and equity < 0.78:
                return Move.CHECK if Move.CHECK in valid_moves else Move.FOLD
            ...
            if facing_allin:
                if is_river and equity < 0.74:
                    return Move.CHECK if Move.CHECK in valid_moves else Move.FOLD
                if not is_river and equity < 0.70 and not strong_draw:
                    return Move.CHECK if Move.CHECK in valid_moves else Move.FOLD
            ...
            # Medium strength raise: only with specific conditions
            can_raise_medium = (
                not is_river and
                not facing_heat and
                (strong_draw or overpair) and
                not texture['dangerous']
            )
            if equity >= 0.62 and can_raise_medium:
                _, opp_c = self.commitments(round_history)
            ...
            if facing_heat:
                call_threshold += 0.05
            if strong_draw and not is_river:
                call_threshold -= 0.05
            ...
            if equity + call_bonus >= call_threshold and to_call <= max_call_amount and Move.CALL in valid_moves:
                return Move.CALL
        ...
        # Reduce bonus on wet boards
        if texture['monotone'] or texture['dangerous']:
            effective_equity = equity + (effective_equity - equity) * 0.5

        if self._is_button:
            # IP: wider c-bet, smaller sizing
            if effective_equity >= 0.66:
                sizing = 0.55 if texture['high_card'] and not texture['connected'] else 0.70
                return self.aggressive_action(valid_moves, min_bet, max_bet,
                                              max(min_bet, int(pot * sizing)), Move.CHECK)
            ...
            if strong_draw and not is_river and not opp_loose:
                return self.aggressive_action(valid_moves, min_bet, max_bet,
    ...
    def move(self, community_cards, valid_moves, round_history, min_bet, max_bet):
        # Detect new hand: preflop with no history means reset
        if not community_cards and len(round_history) <= 1:
            self._carried_pot = 0
            self._last_board_len = 0
            self._last_round_history = []
            # Detect position
        ...
        # Sync street state (detect street transitions)
        ...
        pot = self.real_pot(round_history)
