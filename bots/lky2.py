...
class MyPlayer(Player):
    ...
    def postflop_action(
        ...
        if facing_bet:
            ...
            if equity >= 0.62:
                if not facing_allin and Move.RAISE in valid_moves:
                    return (Move.RAISE, min(max_bet, max(min_bet, int(pot * 0.5))))
                return Move.CALL

            # Pot-odds-based calling
            ...
            if equity >= call_threshold and Move.CALL in valid_moves:
                return Move.CALL
        ...
        if equity >= 0.45 and Move.BET in valid_moves:
            return (Move.BET, min(max_bet, max(min_bet, int(pot * 0.35))))

        # Semi-bluff with draws (not vs loose opponents who call everything)
        if has_draw and not opp_loose and Move.BET in valid_moves:
            return (Move.BET, min(max_bet, max(min_bet, int(pot * 0.4))))

        # High card bluff (exploit fold equity, not vs loose)
        if equity < 0.35 and not opp_loose and Move.BET in valid_moves:
